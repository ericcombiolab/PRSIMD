...
def get_dcoding_byDataField(mapping_table, data_field = None):
    ...
    if len(coding) > 1:
        print('data type error: unmatched data-coding for one data field, check the selected columns!')
        import sys
        sys.exit()
    else:
        coding = coding[0]
...
def get_dtype_byDataField(mapping_table, data_field = None):
    ...
    if len(type)==0: ## make sure the data field in the csv file
        ...
        print('data type error: unmatched types for one data field, check the selected columns!')
        import sys
        sys.exit()
    else:        
        type = type[0]
...
if __name__ == '__main__':
    ...
    categoryTable_path = '/home/comp/csyuxu/PRSIMD/code/non_genetic_data/category_DataField_tables'
    
    category_map = { 1001:'Primary_demographics', 704:'Lifestyle', 706:'Physical_measures',100069:'Female_specific'}
    category_ID = 100069 # modify here for different category in UKBB
    categoryTable = pd.read_table(os.path.join(categoryTable_path, str(category_ID) +'_'+ category_map[category_ID]+'.txt'))
    ...
    mat = pd.DataFrame(np.array(collect),columns=['data_field','col_start','col_end','data_type','data_coding','field_name'])
    
    ## saving for downstream operation
    mat.to_csv('processed_'+str(category_ID)+'_'+ category_map[category_ID] +'.txt',sep='\t',index=False)
