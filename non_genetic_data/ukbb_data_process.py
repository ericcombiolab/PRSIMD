...
def get_Coding_Items_list_forOneType(type_coding, dir, type='Integer'):
    ...
    if all_coding:
        coding_items_list = get_Coding_Items_list(dir, all_coding) # -1: do not know; -3: prefer not to answer --> taking them as missing values   
...
def maually_check_type_coding():
    ...
    ## for Integer type 
    coding_items_list = get_Coding_Items_list_forOneType(type_coding, codings_dir, type='Integer') # -10: half-> convert the values to 0.5
...
if __name__ == '__main__':   
    ...
    disease = 'cad'
    category = 'primary_demographics'    # primary_demographics, lifestyle, physical_measures
    codings_dir = './dataCoding_tables'
    ...
    for _set in ['train','test','val']:
        eid = pd.read_table(os.path.join(eid_path, disease+'_'+_set+'_eid.txt'),header=None).values.ravel()
        idx = []
        for i in eid:
        ...
        tmp = mat.iloc[idx,:]
        tmp.to_csv(os.path.join(save_dir_, category+'_'+ _set +'_data.txt'),sep='\t',index=None)
        print(tmp)
