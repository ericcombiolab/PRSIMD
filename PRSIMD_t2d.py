...
class model(nn.Module):
    ...
    def __init__(self, n_features):
        ...
        self.w_blood_Creatinine = nn.Parameter(torch.FloatTensor(torch.randn(2,1)))
        ...
        self.w_family_diabetes = nn.Parameter(torch.FloatTensor(torch.randn(2,1))) 
        self.w_family_highbp = nn.Parameter(torch.FloatTensor(torch.randn(2,1))) 
        self.w_family_stroke = nn.Parameter(torch.FloatTensor(torch.randn(2,1))) 
        self.w_family_heart = nn.Parameter(torch.FloatTensor(torch.randn(2,1)))  
        self.w_under_COPD = nn.Parameter(torch.FloatTensor(torch.randn(2,1))) 
    ...
    def forward(self,f_data):
        ...
        # Blood pressure
        Diastolic = f_data[:,7].unsqueeze(0)
        Systolic = f_data[:,8].unsqueeze(0)
        bpress = self.w_bp_Diastolic.reshape(2,1) @ Diastolic + self.w_bp_Systolic.reshape(2,1) @ Systolic
        bpress = torch.sigmoid(bpress)
        bpress = self.M[:,6].reshape(2,1) * bpress
        # Sleep duration
        sleep = dt.one_hot(f_data[:,9].unsqueeze(0),2)
        sleep = self.w_sleep @ sleep.T # v_w
        sleep = torch.sigmoid(sleep)
        sleep = self.M[:,7].reshape(2,1) * sleep # v_gamma * const()
        # Bload chemistry
        blood_Creatinine = f_data[:,10].unsqueeze(0)
        blood_Cholesterol = f_data[:,11].unsqueeze(0)
        blood_Triglycerides = f_data[:,12].unsqueeze(0)
        blood_HDLcholesterol = f_data[:,13].unsqueeze(0)
        blood_assays = (self.w_blood_Creatinine.reshape(2,1) @ blood_Creatinine +
        ...
        blood_assays = self.M[:,8].reshape(2,1) * blood_assays     
        # Family history
        family_diabetes = f_data[:,14].unsqueeze(0)
        family_highBP = f_data[:,15].unsqueeze(0)
        family_stroke = f_data[:,16].unsqueeze(0)
        family_heart = f_data[:,17].unsqueeze(0)
        family_history = (
            self.w_family_diabetes.reshape(2,1) @ family_diabetes +
            self.w_family_highbp.reshape(2,1) @ family_highBP +
            self.w_family_stroke.reshape(2,1) @ family_stroke +
            self.w_family_heart.reshape(2,1) @ family_heart
        )
        family_history = torch.sigmoid(family_history)
        family_history = self.M[:,9].reshape(2,1) * family_history
        # Underlying history
        Ud_COPD = f_data[:,18].unsqueeze(0)
        underly_dise = (
            self.w_under_COPD.reshape(2,1) @ Ud_COPD 
        )
        underly_dise = torch.sigmoid(underly_dise)
        underly_dise = self.M[:,10].reshape(2,1) * underly_dise

        f=(
            age + gender + bmi + smoke + alcohol + physical + bpress + sleep + blood_assays + underly_dise + family_history
        )
...
def model_train(data,label,PRSore,val_data,val_label,val_PRSore,num_features,
    ...
    for epoch in range(n_epoch):
        ...
        for i in batches:
            ...
            # loss calculation
            kl = dt.kl_loss_score(result[:,1], result_prs) # KL-Divergence
    ...
    np.save(val_loss_save+'val_loss_t2d_'+str(alpha)+'_'+str(beta), val_loss_epoch)
...
if __name__ == '__main__':
    ...
    work_path = os.getcwd()
    ## RPS score Loading ##
    PRS_tr = work_path + '/PRS/gw_train_t2d.score'
    PRS_val = work_path + '/PRS/gw_vali_t2d.score'
    PRS_te = work_path + '/PRS/gw_test_t2d.score'
    ...
    train = np.load(work_path + '/dataset/train_t2d.npy')
    val = np.load(work_path + '/dataset/validation_t2d.npy')
    test = np.load(work_path + '/dataset/test_t2d.npy')
    # labels  loading ##
    y_train = np.load(work_path + '/dataset/train_t2d_y.npy')
    y_val = np.load(work_path + '/dataset/validation_t2d_y.npy')
    y_test = np.load(work_path + '/dataset/test_t2d_y.npy')
    # Torch data format ## 
    ...
    inputFeatures= 11 # number of phenotype data categories.
