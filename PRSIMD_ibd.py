...
class model(nn.Module):
    ...
    def __init__(self, n_features):
        ...
        self.w_food_Bread = nn.Parameter(torch.FloatTensor(torch.randn(2,1)))
            # yes: 1 or no: 0 . If not, w*0
        self.w_drug_Aspirin = nn.Parameter(torch.FloatTensor(torch.randn(2,1))) 
        self.w_drug_VitaminD = nn.Parameter(torch.FloatTensor(torch.randn(2,1)))               
        # weight for categorical feature
        self.w_gender = nn.Parameter(torch.FloatTensor(torch.randn(2,2)))                       
        self.w_smoke = nn.Parameter(torch.FloatTensor(torch.randn(2,3))) #  UKBB Data-Coding 90
        self.w_alcohol = nn.Parameter(torch.FloatTensor(torch.randn(2,3))) #  UKBB Data-Coding 90
        self.w_sleep = nn.Parameter(torch.FloatTensor(torch.randn(2,2)))
        
        self.w_food_Beef = nn.Parameter(torch.FloatTensor(torch.randn(2,6))) #  UKBB Data-Coding 100377
        self.w_food_Lamb = nn.Parameter(torch.FloatTensor(torch.randn(2,6))) #  UKBB Data-Coding 100377
        self.w_food_Pork = nn.Parameter(torch.FloatTensor(torch.randn(2,6))) #  UKBB Data-Coding 100377
    ...
    def forward(self,f_data):
        ...
        # Alcoho status
        alcohol = dt.one_hot(f_data[:,4].unsqueeze(0),3)
        alcohol = self.w_alcohol @ alcohol.T
        alcohol = torch.sigmoid(alcohol)
        alcohol = self.M[:,4].reshape(2,1) * alcohol
        # Physical activity
        phys_mode = f_data[:,5].unsqueeze(0)
        phys_vigo = f_data[:,6].unsqueeze(0)
        physical = self.w_phys_mode.reshape(2,1) @ phys_mode + self.w_phys_vigo.reshape(2,1) @ phys_vigo
        physical = torch.sigmoid(physical)
        physical = self.M[:,5].reshape(2,1) * physical
        # Sleep duration
        sleep = dt.one_hot(f_data[:,7].unsqueeze(0),2)
        sleep = self.w_sleep @ sleep.T # v_w
        sleep = torch.sigmoid(sleep)
        sleep = self.M[:,6].reshape(2,1) * sleep # v_gamma * const()
        # Drug intake
        drug_Aspirin =  f_data[:,8].unsqueeze(0)
        drug_VitaminD =  f_data[:,9].unsqueeze(0)
        drug_intake = (
            self.w_drug_Aspirin.reshape(2,1) @ drug_Aspirin +
            self.w_drug_VitaminD.reshape(2,1) @ drug_VitaminD  
        )
        drug_intake = torch.sigmoid(drug_intake)
        drug_intake = self.M[:,7].reshape(2,1) * drug_intake
        # Food intake
        food_Bread =  f_data[:,11].unsqueeze(0)              # UK biobank: data-coding 100373
        food_Beef =  dt.one_hot(f_data[:,12].unsqueeze(0),6)
        food_Lamb =  dt.one_hot(f_data[:,13].unsqueeze(0),6)
        food_Pork =  dt.one_hot(f_data[:,14].unsqueeze(0),6)
        food_intake = (
            #self.w_food_fish @ food_fish.T +
            self.w_food_Bread.reshape(2,1) @ food_Bread +
            self.w_food_Beef @ food_Beef.T +
            self.w_food_Lamb @ food_Lamb.T +
            self.w_food_Pork @ food_Pork.T  
        )
        food_intake = torch.sigmoid(food_intake)
        food_intake = self.M[:,8].reshape(2,1) * food_intake

        f=(
            age + gender + bmi + smoke + alcohol + physical  + sleep + drug_intake  + food_intake
        )
...
def model_train(data,label,PRSore,val_data,val_label,val_PRSore,num_features,
    ...
    for epoch in range(n_epoch):
        ...
        if val_acc > val_acc_best:
            ...
            torch.save(obj= state, f= param_save+'model_param_g_ibd_'+str(alpha)+'_'+str(beta)+'.pth')
    # for epoch error plot     
    np.save(train_loss_save+'train_loss_ibd_'+str(alpha)+'_'+str(beta), train_loss_epoch)
    np.save(val_loss_save+'val_loss_ibd_'+str(alpha)+'_'+str(beta), val_loss_epoch)
...
def evaluation_auc(PRS_model,model,data,label,PRS,param_path,alpha_test,beta_test,sets='noSpeci',saveResult=False,result_folder=None):
    ...
    trained_param = param_path+'model_param_g_ibd_'+str(alpha_test)+'_'+str(beta_test)+'.pth'
...
if __name__ == '__main__':
    ...
    work_path = os.getcwd()
    ## RPS score Loading ##
    PRS_tr = work_path + '/PRS/gw_train_ibd.score'
    PRS_val = work_path + '/PRS/gw_vali_ibd.score'
    PRS_te = work_path + '/PRS/gw_test_ibd.score'
    ...
    train = np.load(work_path + '/dataset/train_ibd.npy')
    val = np.load(work_path + '/dataset/validation_ibd.npy')
    test = np.load(work_path + '/dataset/test_ibd.npy')
    # labels  loading ##
    y_train = np.load(work_path + '/dataset/train_ibd_y.npy')
    y_val = np.load(work_path + '/dataset/validation_ibd_y.npy')
    y_test = np.load(work_path + '/dataset/test_ibd_y.npy')
    # Torch data format ## 
    ...
    inputFeatures= 9 # feature categories.
    # result folder 
    result = work_path + '/result'
    if result is not None:
        if not os.path.isdir(result):
            os.mkdir(result)
    result_folder = work_path + '/result/ibd_200epoch_monitor_acc'
