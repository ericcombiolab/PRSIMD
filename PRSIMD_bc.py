...
class model(nn.Module):
    ...
    def __init__(self, n_features):
        ...
        self.w_blood_IGF_1 = nn.Parameter(torch.FloatTensor(torch.randn(2,1)))
        self.w_blood_Glucose = nn.Parameter(torch.FloatTensor(torch.randn(2,1))) 
            # yes: 1 or no: 0 . If not, w*0
        self.w_family_bc = nn.Parameter(torch.FloatTensor(torch.randn(2,1))) 
        ...
        self.w_smoke_SHand = nn.Parameter(torch.FloatTensor(torch.randn(2,3)))
    ...
    def forward(self,f_data):
        ...
        # Smoke status
        smoke_1 = dt.one_hot(f_data[:,3].unsqueeze(0),3)
        smoke_2 = dt.one_hot(f_data[:,4].unsqueeze(0),3)
        smoke = self.w_smoke @ smoke_1.T + self.w_smoke_SHand @ smoke_2.T
        smoke = torch.sigmoid(smoke)
        smoke = self.M[:,2].reshape(2,1) * smoke
        # Physical activity
        ...
        physical = self.M[:,3].reshape(2,1) * physical
        # Bload chemistry
        blood_IGF_1 = f_data[:,7].unsqueeze(0)
        blood_Glucose = f_data[:,8].unsqueeze(0)
        blood_assays = (
                        self.w_blood_IGF_1.reshape(2,1) @ blood_IGF_1 +
                        self.w_blood_Glucose.reshape(2,1) @ blood_Glucose
        )
        blood_assays = torch.sigmoid(blood_assays)
        blood_assays = self.M[:,4].reshape(2,1) * blood_assays     
        # Family history
        family_bc = f_data[:,9].unsqueeze(0)
        family_history = (
            self.w_family_bc.reshape(2,1) @ family_bc
        )
        family_history = torch.sigmoid(family_history)
        family_history = self.M[:,5].reshape(2,1) * family_history

        f=(
            age + bmi + smoke + physical + blood_assays + family_history
        )
...
def model_train(data,label,PRSore,val_data,val_label,val_PRSore,num_features,
    ...
    for epoch in range(n_epoch):
        ...
        for i in batches:
            ...
            # loss calculation
            kl = dt.kl_loss_score(result[:,1], result_prs) 
        ...
        if val_acc > val_acc_best:
            ...
            torch.save(obj= state, f= param_save+'model_param_g_bc_'+str(alpha)+'_'+str(beta)+'.pth')
    # for epoch error plot     
    np.save(train_loss_save+'train_loss_bc_'+str(alpha)+'_'+str(beta), train_loss_epoch)
    np.save(val_loss_save+'val_loss_bc_'+str(alpha)+'_'+str(beta), val_loss_epoch)
...
def evaluation_auc(PRS_model,model,data,label,PRS,param_path,alpha_test,beta_test,sets='noSpeci',saveResult=False,result_folder=None):
    ...
    trained_param = param_path+'model_param_g_bc_'+str(alpha_test)+'_'+str(beta_test)+'.pth'
...
if __name__ == '__main__':
    ...
    work_path = os.getcwd()
    ## RPS score Loading ##
    PRS_tr = work_path + '/PRS/gw_train_bc.score'
    PRS_val = work_path + '/PRS/gw_vali_bc.score'
    PRS_te = work_path + '/PRS/gw_test_bc.score'
    ...
    train = np.load(work_path + '/dataset/train_bc.npy')
    val = np.load(work_path + '/dataset/validation_bc.npy')
    test = np.load(work_path + '/dataset/test_bc.npy')
    # labels  loading ##
    y_train = np.load(work_path + '/dataset/train_bc_y.npy')
    y_val = np.load(work_path + '/dataset/validation_bc_y.npy')
    y_test = np.load(work_path + '/dataset/test_bc_y.npy')
    # Torch data format # 
    ...
    inputFeatures= 6 # feature categories.
    ...
    if result is not None:
        if not os.path.isdir(result):
            os.mkdir(result)
    result_folder = work_path + '/result/bc_200epoch_monitor_acc'
